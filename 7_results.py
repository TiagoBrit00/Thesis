...
def compute_bootstrap_stats(df, method_label):
    ...
    for metric in ["Uniqueness","MarketPotential","Hotness"]:
        agg[f"{metric}_95CI"] = 1.96 * agg[f"{metric}_std"] / np.sqrt(agg["n_runs"].clip(lower=1)) # Avoid div by zero
...
def save_ci_plot(df, results_dir, method_label):
    for metric, mean_col, ci_col in [
        ...
        plt.errorbar(df["idea_id"], df[mean_col], yerr=df[ci_col], fmt="o", capsize=5)
        plt.ylim(0,10)
        plt.title(f"{metric} — Mean ± 95% CI ({method_label})")
        plt.xlabel("Idea ID"); plt.ylabel(metric)
        plt.tight_layout()
